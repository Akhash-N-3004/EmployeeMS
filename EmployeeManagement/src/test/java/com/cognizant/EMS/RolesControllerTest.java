...
public class RolesControllerTest {
	  @Mock
	    ...
	    public void testUpdateRoles() throws ResourceNotFoundException {
	        ...
	        when(rolesService.getRolesById(roleId)).thenReturn(Optional.of(role));
	        when(rolesService.updateRoles(roleId, updatedRole)).thenReturn(updatedRole);

	        Roles result = rolesController.updateRoles(roleId, updatedRole);

	        assertEquals(roleId, result.getId());
	        assertEquals("Updated Role", result.getRole());
	        assertEquals(20000, result.getSalary());
	        verify(rolesService, times(1)).getRolesById(roleId);
	        verify(rolesService, times(1)).updateRoles(roleId, updatedRole);
	    }
	    ...
	    public void testUpdateRoles_NotFound() {
	        ...
	        try {
	            rolesController.updateRoles(roleId, updatedRole);
	        } catch (ResourceNotFoundException e) {
	            assertEquals("Role data is not found", e.getMessage());
	        }
